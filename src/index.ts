...
const evaluateAI = (i: number) => {
    for (const ai of AIs) {
        if (ai.car.hidden || ai.car.isHiding) continue;
        ...
        if (i === 0) {
            ...
            if (car.color === 'green') bestAI = ai;
            if (car.color === 'blue') secondBestAI = ai;
        }
    ...
    if (secondBestAI !== undefined) secondBestAI.car.render();
    if (bestAI !== undefined) bestAI.car.render();
}
...
const nextGen = () => {
    if (deadAIAmount === AIAmount) {
        ...
        if (selectedParent1 === undefined) throw new Error();

        // get the second parent
        let fitnessSum2 = 0;
        // calculate the sum of fitnesses
        for (const ai of AIs) {
            if (ai.getFitness() === selectedParent1.getFitness()) continue;
        ...
        let selectedParent2;
        for (let i = 0; i < AIs.length; i++) {
            if (AIs[i] === selectedParent1) continue;
        ...
        if (selectedParent2 === undefined) throw new Error();

        const arr: Array<AI> = selectedParent1.produceNChildren(selectedParent2, AIs.length - 2);
        AIs = [];
